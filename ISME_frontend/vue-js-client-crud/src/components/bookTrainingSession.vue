  ...
  <div class="aselect" :data-value="branchValue" :data-list="list">
    <div class="selector" @click="toggleBranchSelector()">
      ...
      <div :class="{ hidden : !visibleBranchArrow, visibleBranchArrow }">
        <ul>
          <li :clsass="{ current : item === branchValue }" v-for="item in dropDownBranches" :key="item" @click="selectBranch(item)">{{ item }}</li>
        </ul>
        </div>
      </div>
	</div>
    ...
    <div class="selector" @click="toggleEmployeeSelector()">
      ...
      <div :class="{ hidden : !visibleEmployeeArrow, visibleEmployeeArrow }">
        <ul>
          <li :clsass="{ current : item === employeeValue }" v-for="item in dropDownEmployees" :key="item" @click="selectEmployee(item)">{{ item }}</li>
        </ul>
        </div>
      </div>
	</div>
    ...
    <button class="btn btn-primary" type="button" v-on:click="bookTrainingSession()">book Session</button>
    <div class="col-md-6">
    ...
    el: "#app",
      ...
      getEmployeesForBranch(){
          if(this.branch !== ""){
            ...
            FitnessCenterService.getEmployeesForBranch(branch.city, branch.zip ,branch.street)
                     .then(response =>{
                      this.employees = response.data;
                      for(const employee of this.employees){
                        this.dropDownEmployees.push(employee.svnr + " " + employee.firstName);
                      }
                   }).catch(e => {
                      console.log(e);
                  });
      ...
      bookTrainingSession() {
        if(this.employee !== "" && this.duration !== null  && this.price !== null){
          for(const employee of this.employees){
              if(this.employee.includes(employee.svnr)){
                 var trainingsession = {
                   "member_svnr": LoginService.currentUserValue.svnr,
                   "employee_svnr": employee.svnr,
                   "price": price,
                   "duration" : duration
                 }
                 FitnessCenterService.bookTrainingSession(JSON.stringify(trainingsession))
                 .then(response => {
      ...
      getMembersTrainingSessions() {
        FitnessCenterService.getMembersTrainingSessions(LoginService.currentUserValue.svnr)
            .then(response => {
