      ...
      FitnessCenterService.getMembersRegistredBranches(
        ...
        .then((response) => {
          ...
          for (const registerdBranch of registredbranchesResponse) {
            this.registerdBranches.push(
              registerdBranch.name +
                "-" +
                registerdBranch.street +
                " " +
                registerdBranch.zip +
                " " +
                registerdBranch.city
            );
          }
        })
        .catch((e) => {
      ...
      if (this.branch !== "") {
        for (const branch of this.branches) {
          if (
            ...
            FitnessCenterService.registerForBranch(
              branch.city,
              branch.zip,
              branch.street,
              LoginService.loggedUserValue.svnr
